...
# database connection for validation credential
def databaseConnection2():
   CONNECTION_STRING = "mongodb+srv://emseccomandcenter:TUXnEN09VNM1drh3@cluster0.psiqanw.mongodb.net/?retryWrites=true&w=majority"
   client = MongoClient(CONNECTION_STRING)
   return client['ValidationPortal']
db2 = databaseConnection2()

# XPATH collection 
xpath_collection = db["Xpath_forum"]

# LoginCredential collection 
login_collection = db2["credentials"]
...
#ALL Links
@app.route('/forumLinks',methods =['GET'])
def ForumLinks():
    if(request.method=='GET'):
        # query_args = request.args
        # page = int(query_args['page'])
        # page_limit = int(query_args['limit'])
        # list_data = link_collection.find().sort('_id', -1).skip((page-1)*page_limit).limit(page_limit)
        page =18
        page_limit =5
        list_data = link_collection.find().sort('_id', -1).skip((page-1)*page_limit).limit(page_limit)
        try:    
            # fLinks =link_collection.find().limit(2)
            # linksData =dumps(fLinks)
            linksData =dumps(list_data)
            return linksData
        except:
            resp = jsonify("No Data")
            resp.status_code = 204
...
# GET , PUT and UPDATE LoginCredential
@app.route('/forumLinks/<id>',methods=['GET','PATCH'])
def ForumLink(id):
    if (request.method =='GET'):
        try:
